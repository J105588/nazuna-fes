    ...
    <div className="space-y-8 select-none animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <form
          ...
          className="lg:col-span-3 bg-white border border-slate-200 rounded-2xl p-6 space-y-5 shadow-sm flex flex-col justify-between"
        >
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-700">配信タイトル</label>
              <input
                type="text"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder="例: 【速報】中庭ステージ開演10分前"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-amber-500 transition-all"
                required
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-700">配信カテゴリ</label>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-700">本文メッセージ</label>
              <textarea
                rows={4}
                value={content}
                onChange={(e) => setContent(e.target.value)}
                placeholder="配信する詳細なテキストや案内事項を入力してください。"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-amber-500 leading-relaxed transition-all"
                required
              />
            </div>
          </div>
        ...
        {/* ライブプレビュー */}
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 space-y-4 flex flex-col justify-center relative overflow-hidden shadow-sm">
          ...
          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
            ...
            <span className="text-[10px] text-slate-500 font-mono">ユーザー画面プレビュー</span>
          </div>

          <div className="space-y-2">
            <p className="text-xs text-slate-500">以下のようにユーザーに表示されます：</p>
            <div
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-[10px] font-mono text-slate-500">たった今</span>
              </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-8 pr-3 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-amber-500 transition-all font-mono"
            />
            {searchQuery && (
              <button
        ...
        {filteredAnnouncements.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center space-y-3 shadow-sm">
            ...
            <h3 className="font-bold text-sm text-slate-800">該当するお知らせが見つかりませんでした</h3>
            <p className="text-xs text-slate-500">条件を変更するか、新しいお知らせを配信してください。</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="space-y-2.5">
                    ...
                    <h4 className="font-bold text-base text-slate-900">{ann.title}</h4>
                  ...
                  <div className="pt-3 border-t border-slate-200 flex items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-slate-100 hover:bg-red-50 text-slate-600 hover:text-red-600 transition-all border border-slate-200"
                      title="削除"
                    >
